...
# objective function
tmpS <- function(Y,add_x,U,V,thetahat,bHat1,bHat2,alphahat,L2NNer,lambda,tau,M,gamma,h)
{
  bHat <- c(bHat1,bHat2)
  n <- length(Y)
  loss <- 0
  ...
  for (ii in 1:n)
  {
    tmp <- Y[ii] -t(add_x[ii,])%*%thetahat-t(U[ii,])%*%bHat-alphahat
...
bHat <- initial_est[-c(1:(nonfunc+1))]
thetahat <- initial_est[2:(nonfunc+1)]
alphahat <- initial_est[1]
bHat1 <- bHat[1:ncol(U1)]
bHat2 <- bHat[1:ncol(U2)+ncol(U1)]
minabs <- sort(abs(c(thetahat,bHat)))[which(sort(abs(c(thetahat,bHat)))>0)[1]]
epsilon <- min(tol/(2*n*Dpfunc(0,lambda[1],a)),tol/(2*n*Dpfunc(0,lambda[2],a)))*minabs
...
while(change>=tol)
{
  ...
  lqaW = lqaW/2
  tmpchange <- 1
  tmpobj2 <- tmpS(Y,add_x,U,V,thetahat,bHat1,bHat2,alphahat,L2NNer,lambda,tau,M,gamma,h)
  ...
  while(tmpchange>=tol)
  {
    ...
    s = 0
    for (ii in 1:n){
      ds <- ds + dGh(Y[ii]-sum(add_x[ii,]*thetahat)-sum(U[ii,]*bHat)-alphahat,h)*as.matrix(c(1,add_x[ii,],U[ii,]))%*%c(1,add_x[ii,],U[ii,])
      s <- s-(Gh(Y[ii]-sum(add_x[ii,]*thetahat)-sum(U[ii,]*bHat)-alphahat,h)+tau-1)*c(1,add_x[ii,],U[ii,])
    }
    s <- as.matrix(s)
    tmpmatrix <- bdiag(matrix(0,nrow=nonfunc+1, ncol=nonfunc+1),n*lqaW + bdiag(gamma[1]*V,gamma[2]*V))
    tmpmatrix <- tmpmatrix + ds
    tmp <- svd(tmpmatrix)
    tmpinv <- (tmp$u)%*%diag((tmp$d)^-1)%*%t(tmp$v)
    scjk <- t(bHat)%*%(n*lqaW)+ t(bHat)%*%bdiag(gamma[1]*V,gamma[2]*V)
    sdiff <- -1
    ...
    while(sdiff < 0)
    {
      ...
      tmpobj2 <- tmpS(Y,add_x,U,V,tmptheta,tmpbHat1,tmpbHat2,tmpalphahat,L2NNer,lambda,tau,M,gamma,h)
...
# estimated null subregions of the functional covariates
